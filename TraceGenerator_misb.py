...
if __name__ == '__main__':    
    opts, args = getopt.getopt(sys.argv[1:],"hl:f:m:",["distrib=", "max=", "start="])
    for opt, arg in opts:
        if opt == '-h':
            print 'python TraceGenerator.py -l <low> -f <full> -m <misbehave>'
            sys.exit()
        elif opt == '-l':
            low = float(arg)    # in MB/s
            low_iops = low * 1024 / iosize
        elif opt == '-f':
            full = float(arg)    # in MB/s
            full_iops = full * 1024 / iosize
        elif opt == '-m':
            misb = float(arg)    # in MB/s
            misb_iops = misb * 1024 / iosize

    stdv_iops = 0.1 * 1024 / iosize
    resume_iops = full_iops

    low_len = 180  #3min
    misb_len = 540  #9min
    resume_len = 180 #3min
    iops_array = np.random.normal(low_iops,stdv_iops,low_len)
    
    iops = np.random.normal(misb_iops,stdv_iops,misb_len)
    iops_array = np.append(iops_array, iops)

    iops = np.random.normal(resume_iops,stdv_iops,resume_len)
    iops_array = np.append(iops_array, iops)
